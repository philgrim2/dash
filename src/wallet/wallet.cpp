...
/**
 * Fees smaller than this (in notions) are considered zero fee (for transaction creation)
            ...
            while (--nMaxTries > 0)
            {
                ...
                if (getChange() < 0) {
                    if (nSubtractFeeFromAmount == 0) {
                        ...
                        // so just add 1 notions to fail in the next loop step with a correct reason
