...
void TransactionView::changedAmount()
{
    ...
    // Replace "," by "." so ThoughtUnits::parse will not fail for users entering "," as decimal separator
    QString newAmount = amountWidget->text();
    newAmount.replace(QString(","), QString("."));

    if (ThoughtUnits::parse(model->getOptionsModel()->getDisplayUnit(), newAmount, &amount_parsed)) {
...
void TransactionView::exportClicked()
{
    ...
    writer.addColumn(ThoughtUnits::getAmountColumnTitle(model->getOptionsModel()->getDisplayUnit()), 0, TransactionTableModel::FormattedAmountRole);
...
void TransactionView::showAddressQRCode()
{
    ...
    dialog->setInfo(tr("QR code"), "thought:"+strAddress, "", strAddress);
    dialog->show();
}
...
void TransactionView::computeSum()
{
    ...
    QString strAmount(ThoughtUnits::formatWithUnit(nDisplayUnit, amount, true, ThoughtUnits::separatorAlways));
