...
{
    GUIUtil::setClipboard(ThoughtUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::updateLabels(CCoinControl& m_coin_control, WalletModel *model, QDialog* dialog)
{
    ...
    int nDisplayUnit = ThoughtUnits::THT;
    ...
    l2->setText(ThoughtUnits::formatWithUnit(nDisplayUnit, nAmount));        // Amount
    l3->setText(ThoughtUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(ThoughtUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes));        // Bytes
    l7->setText(fDust ? tr("yes") : tr("no"));                               // Dust
    l8->setText(ThoughtUnits::formatWithUnit(nDisplayUnit, nChange));        // Change
    if (nPayFee > 0)
    {
    ...
    QString toolTip4 = tr("Can vary +/- %1 notion(s) per input.").arg(dFeeVary);
    ...
    for (const auto& coins : model->wallet().listCoins()) {
        ...
        for (const auto& outpair : coins.second) {
            ...
            if (ExtractDestination(out.txout.scriptPubKey, outputAddress)) {
                ...
                // if listMode or change => show thought address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, ThoughtUnits::format(nDisplayUnit, out.txout.nValue));
            itemOutput->setToolTip(COLUMN_AMOUNT, ThoughtUnits::format(nDisplayUnit, out.txout.nValue));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, ThoughtUnits::format(nDisplayUnit, nSum));
            itemWalletAddress->setToolTip(COLUMN_AMOUNT, ThoughtUnits::format(nDisplayUnit, nSum));
