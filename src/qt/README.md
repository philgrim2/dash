This directory contains the ThoughtQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).
...
Represents the main window of the Thought UI.
...
Used to process BIP21 and BIP70 (see https://github.com/bitcoin/bitcoin/pull/11622) payment URI / requests. Also handles URI based application switching (e.g. when following a thought:... link from a browser).
...
* UI elements like ThoughtAmountField, which inherit from QWidget.
...
* don't change `local/thought_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
10. Start debugging with Qt Creator (you might need to the executable to "thought-qt" under "Run", which is where you can also add command line arguments)
