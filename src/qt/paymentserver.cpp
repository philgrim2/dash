...
const int THOUGHT_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString THOUGHT_IPC_PREFIX("thought:");
// BIP70 payment protocol messages
const char* BIP70_MESSAGE_PAYMENTACK = "PaymentACK";
const char* BIP70_MESSAGE_PAYMENTREQUEST = "PaymentRequest";
// BIP71 payment protocol media types
const char* BIP71_MIMETYPE_PAYMENT = "application/thought-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/thought-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/thought-paymentrequest";
...
void PaymentServer::ipcParseCommandLine(interfaces::Node& node, int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the thought: URI contains a payment request, we are not able to detect the
        // network as that would require fetching and parsing the payment request.
        // That means clicking such an URI which contains a testnet payment request
        // will start a mainnet instance and throw a "wrong network" error.
        if (arg.startsWith(THOUGHT_IPC_PREFIX, Qt::CaseInsensitive)) // thought: URI
        {
            savedPaymentRequests.append(arg);

            SendCoinsRecipient r;
            if (GUIUtil::parseThoughtURI(arg, &r) && !r.address.isEmpty())
            {
    ...
    optionsModel(0)
{
    ...
    // on Mac: sent when you click thought: links
...
//
// OSX-specific way of handling thought: URIs and PaymentRequest mime types.
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in thought: URIs
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("thought://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'thought://' is not a valid URI. Use 'thought:' instead."),
            CClientUIInterface::MSG_ERROR);
    }
    else if (s.startsWith(THOUGHT_IPC_PREFIX, Qt::CaseInsensitive)) // thought: URI
    {
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseThoughtURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid Thought address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    for (const std::pair<CScript, CAmount>& sendingTo : sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            // Unauthenticated payment requests to custom thought addresses are not supported
        ...
        // Thought amounts are stored as (optional) uint64 in the protobuf messages (see paymentrequest.proto),
        ...
        if (IsDust(txOut, optionsModel->node().getDustRelayFee())) {
            Q_EMIT message(tr("Payment request error"), tr("Requested payment amount of %1 is too small (considered dust).")
                .arg(ThoughtUnits::formatWithUnit(optionsModel->getDisplayUnit(), sendingTo.second)),
                CClientUIInterface::MSG_ERROR);
