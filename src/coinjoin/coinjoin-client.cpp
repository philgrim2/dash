...
bool CCoinJoinClientSession::SignFinalTransaction(const CTransaction& finalTransactionNew, CNode* pnode, CConnman& connman)
{
    ...
    for (const auto& entry : vecEntries) {
        ...
        for (const auto& txdsin : entry.vecTxDSIn) {
            ...
            // TODO we're using amount=0 here but we should use the correct amount. This works because thoughtcore ignores the amount while signing/verifying (only used in Bitcoin/Segwit)
...
bool CCoinJoinClientSession::MakeCollateralAmounts(const CompactTallyItem& tallyItem, bool fTryDenominated)
{
    ...
    if (txBuilder.CouldAddOutputs({CCoinJoin::GetMaxCollateralAmount(), CCoinJoin::GetCollateralAmount()})) {
        ...
        // If remainder is denominated add one notion to the fee
        ...
        // Create two equal outputs from the available value. This adds one notion to the fee if txBuilder.GetAmountLeft() is odd.
