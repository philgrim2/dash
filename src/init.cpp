...
public:
    ...
    // Thought Specific WalletInitInterface InitCoinJoinSettings
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-conf=<file>", strprintf("Specify configuration file. Relative paths will be prefixed by datadir location. (default: %s)", THOUGHT_CONF_FILENAME), false, OptionsCategory::OPTIONS);
...
#ifndef WIN32
    gArgs.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", THOUGHT_PID_FILENAME), false, OptionsCategory::OPTIONS);
#endif
    ...
    gArgs.AddArg("-sporkaddr=<thoughtaddress>", "Override spork address. Only useful for regtest and devnet. Using this on mainnet or testnet will ban you.", false, OptionsCategory::DEBUG_TEST);
    ...
    gArgs.AddArg("-platform-user=<user>", "Set the username for the \"platform user\", a restricted user intended to be used by Thought Platform, to the specified username.", false, OptionsCategory::MASTERNODE);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/thoughtnetwork/thoughtcore>";
...
bool AppInitMain()
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", THOUGHT_CONF_FILENAME)).string());
    LogPrintf("Using at most %i automatic connections (%i file descriptors available)\n", nMaxConnections, nFD);

    // Warn about relative -datadir path.
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if Thought Core is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if Thought Core is started while in a temporary directory.\n",
    ...
    // ********************************************************* Step 10c: schedule Thought-specific tasks
