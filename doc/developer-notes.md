...
- [Development guidelines](#development-guidelines)
    - [General Thought Core](#general-thought-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Thought Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/thoughtnetwork/thoughtcore/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_thought
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_thought --log_level=test_suite
$ valgrind -v --leak-check=full src/thoughtd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_thought.coverage/index.html`.
```

**Sanitizers**

Thought Core can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as Thought Core, where everyone uses
...
pay attention to for reviewers of Thought Core code.

General Thought Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Thought Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
- Use include guards to avoid the problem of double inclusion. The header file
  `foo/bar.h` should use the include guard identifier `THOUGHT_FOO_BAR_H`, e.g.

```c++
#ifndef THOUGHT_FOO_BAR_H
#define THOUGHT_FOO_BAR_H
...
#endif // THOUGHT_FOO_BAR_H
```
...
be sent upstream but bugfixes may also be prudent to PR against Thought Core so that they can be integrated
...
file descriptors it will cause problems with Thought's `select()` loop, because
...
would be to revert the upstream fix before applying the updates to Thought's
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `thought-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `thought-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Thought Core, the wallet was always
